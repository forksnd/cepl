...
(defn-inline uniform-1ui ((location (signed-byte 32))
                          (value (unsigned-byte 32)))
    (values)
  (declare (optimize (speed 3) (safety 1) (debug 0)
                     (compilation-speed 0))
           (profile t))
  #+sbcl(declare (sb-ext:muffle-conditions sb-ext:compiler-note))
  (%gl:uniform-1ui location value)
  (values))
...
(defn-inline uniform-2ui ((location (signed-byte 32))
                          (value uvec2))
    (values)
  (declare (optimize (speed 3) (safety 1) (debug 0)
                     (compilation-speed 0))
  ...
  (cffi-sys:with-pointer-to-vector-data (ptr value)
    #+sbcl(declare (sb-ext:muffle-conditions sb-ext:compiler-note))
    (%gl:uniform-2uiv location 1 ptr))
  (values))

(defn-inline uniform-3ui ((location (signed-byte 32))
                          (value uvec3))
    (values)
  (declare (optimize (speed 3) (safety 1) (debug 0)
                     (compilation-speed 0))
  ...
  (cffi-sys:with-pointer-to-vector-data (ptr value)
    #+sbcl(declare (sb-ext:muffle-conditions sb-ext:compiler-note))
    (%gl:uniform-3uiv location 1 ptr))
  (values))

(defn-inline uniform-4ui ((location (signed-byte 32))
                          (value uvec4))
    (values)
  (declare (optimize (speed 3) (safety 1) (debug 0)
                     (compilation-speed 0))
  ...
  (cffi-sys:with-pointer-to-vector-data (ptr value)
    #+sbcl(declare (sb-ext:muffle-conditions sb-ext:compiler-note))
    (%gl:uniform-4uiv location 1 ptr))
  (values))
...
(defun+ get-foreign-uniform-function-name (type)
  ...
  (case type
    ...
    ((:ivec2 :int-vec2 :int-vec2-arb :bool-vec2 :bool-vec2-arb) '%gl:uniform-2iv)
    ((:ivec3 :int-vec3 :int-vec3-arb :bool-vec3 :bool-vec3-arb) '%gl:uniform-3iv)
    ((:ivec4 :int-vec4 :int-vec4-arb :bool-vec4 :bool-vec4-arb) '%gl:uniform-4iv)
    ((:uvec2 :uint-vec2) '%gl:uniform-2uiv)
    ((:uvec3 :uint-vec3) '%gl:uniform-3uiv)
    ((:uvec4 :uint-vec4) '%gl:uniform-4uiv)
...
(defun+ get-uniform-function-name (type)
  ...
  (case type
    ...
    ((:ivec2 :int-vec2 :int-vec2-arb :bool-vec2 :bool-vec2-arb) 'uniform-2i)
    ((:ivec3 :int-vec3 :int-vec3-arb :bool-vec3 :bool-vec3-arb) 'uniform-3i)
    ((:ivec4 :int-vec4 :int-vec4-arb :bool-vec4 :bool-vec4-arb) 'uniform-4i)
    ((:uvec2 :uint-vec2) 'uniform-2ui)
    ((:uvec3 :uint-vec3) 'uniform-3ui)
    ((:uvec4 :uint-vec4) 'uniform-4ui)
